locals {
  helm_values = {
    ...
    "resources.requests.cpu"    = 1        # Default of 4 is too large for a 4-CPU node type
    "resources.requests.memory" = "2000Mi" # Default of 5000Mi can lead to scheduling issues with 8GB node type
    ...
    "env.variables.TFE_OBJECT_STORAGE_S3_REGION"                            = var.s3_region
    ...
    "env.variables.TFE_METRICS_ENABLE"                                      = "true"
  }
  ...
  service_account_annotations = { for k, v in var.service_account_annotations : "serviceAccount.annotations.${k}" => v }

  values = merge(local.helm_values, local.service_annotations, local.service_account_annotations)
}
